...
export function UserAvatar() {
  return (
    <div
      ...
      style={{ backgroundColor: "rgb(32, 33, 35)" }}
    ></div>
  );
}

export function UserItem({ content }: { content: React.ReactNode }) {
  return <ItemLayout left={<UserAvatar />}>{content}</ItemLayout>;
}

export function BotItem({ content }: { content: React.ReactNode }) {
  return (
    <ItemLayout
      left={
        <div
...
export function ContentItem(props: ContentItemProps) {
  return (
    <div className="flex flex-col items-center text-sm ">
      <UserItem content={props.question} />
      <BotItem content={props.answer} />
    </div>
  );
}
